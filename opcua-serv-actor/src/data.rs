use actor::{ActorError, ActorResultVoid};
...
impl Node {
    fn process(node: Node, parent: Option<&NodeId>, addr: &mut AddressSpace) -> ActorResultVoid {
        let id = &node.node_id;
        ...
        match &node.delegate {
            NodeDelegate::Folder => {
                if !addr.add_folder_with_id(
                    ...
                    return Err(ActorError::StartupError(format!(
...
impl ServerStructure {
    ...
    pub fn process_server(&self, server: Arc<RwLock<Server>>) -> ActorResultVoid {
        for namespace in &self.namespaces {
            if namespace.0 == 0 {
                return Err(
                    ActorError::StartupError("Namespace index cannot be 0".to_string()).into(),
                );
            }
            if namespace.1.is_empty() {
                return Err(
                    ActorError::StartupError("Namespace URI  cannot be empty".to_string()).into(),
                );
            }
        }
        ...
        for Namespace(idx, uri) in sorted.iter() {
            let result = addr
                ...
                .map_err(|_| ActorError::StartupError("Can not register namespace".to_string()))?;

            if *idx != result {
                return Err(ActorError::StartupError(format!(
                    "Namespace index mismatch {idx} != {result}"
                )));
            }
        }
